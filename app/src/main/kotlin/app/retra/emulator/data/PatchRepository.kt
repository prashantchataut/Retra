...
class PatchRepository @Inject constructor(
    ...
    suspend fun apply(base: GameRecord, patchUri: Uri): PatchOutcome = withContext(Dispatchers.IO) {
        ...
        val title = if (header.title.equals(base.title, ignoreCase = true)) "${base.title} (Patched)" else header.title
        ...
        val id = try {
            ...
            if (gameDao.countBySha256(result.outputSha256) > 0) {
                // The output path is content-addressed and may be used by the winning concurrent row.
